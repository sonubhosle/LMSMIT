...
class StudentsManager {
    ...
    async loadStudents() {
        try {
            ...
            // Debug: Check first student's structure
            if (data.students && data.students.length > 0) {
                console.log('Full student data structure:', JSON.stringify(data.students[0], null, 2));
            }
            ...
            this.totalStudents = data.total || 0;
            
            // Update total students count
            document.getElementById('totalStudentsCount').textContent = this.totalStudents.toLocaleString();
            
            // Get issued books for all students
            const studentsWithIssuedBooks = await this.fetchAllStudentsIssuedBooks(data.students || []);
            
            // Calculate stats
            this.calculateAndUpdateStats(studentsWithIssuedBooks);
            
            // Render table
            this.renderStudentsTable(studentsWithIssuedBooks);
    ...
    // Helper method to get issued books from student object
    getStudentIssuedBooks(student) {
        if (!student) return [];
        
        console.log('Student issuedBooks structure:', student.issuedBooks);
        
        // Case 1: Direct array
        if (Array.isArray(student.issuedBooks)) {
            return student.issuedBooks;
        }
        
        // Case 2: Object with nested array
        if (student.issuedBooks && typeof student.issuedBooks === 'object') {
            // Try common property names
            if (Array.isArray(student.issuedBooks.issuedBooks)) {
                return student.issuedBooks.issuedBooks;
            }
            if (Array.isArray(student.issuedBooks.books)) {
                return student.issuedBooks.books;
            }
            if (Array.isArray(student.issuedBooks.data)) {
                return student.issuedBooks.data;
            }
            if (Array.isArray(student.issuedBooks.transactions)) {
                return student.issuedBooks.transactions;
            }
            if (Array.isArray(student.issuedBooks.issued)) {
                return student.issuedBooks.issued;
            }
            
            // If object has values that might be arrays
            const values = Object.values(student.issuedBooks);
            if (values.length > 0 && Array.isArray(values[0])) {
                return values[0];
            }
            
            // If it's an object with book data directly
            const booksArray = Object.values(student.issuedBooks).filter(item => 
                item && typeof item === 'object' && (item.book || item.title || item.status)
            );
        ...
        if (Array.isArray(student.books)) {
            return student.books;
        }
        if (Array.isArray(student.issued)) {
            return student.issued;
        }
        if (Array.isArray(student.transactions)) {
    ...
    // Fetch student's issued books from transactions/issues API - FIXED
    async fetchStudentIssuedBooks(studentId) {
        try {
            // Try multiple endpoints based on your database structure
            const endpoints = [
                `http://localhost:5000/api/transactions?student=${studentId}`,
                `http://localhost:5000/api/issues?student=${studentId}`,
                `http://localhost:5000/api/transactions/student/${studentId}`,
                `http://localhost:5000/api/issues/student/${studentId}`,
                `http://localhost:5000/api/issued-books?student=${studentId}`
            ];
            ...
            for (const endpoint of endpoints) {
                try {
                    ...
                    if (response.ok) {
                        ...
                        if (data.success) {
                            // Return only active/issued books (not returned ones)
                            const allTransactions = data.transactions || data.issues || data.data || data.issuedBooks || [];
    ...
    // Fetch issued books for all students - NEW METHOD
    async fetchAllStudentsIssuedBooks(students) {
        const studentsWithIssuedBooks = [...students];
        
        // Show loading progress only if we have many students
        const tbody = document.getElementById('studentsTableBody');
        
        for (let i = 0; i < studentsWithIssuedBooks.length; i++) {
            const student = studentsWithIssuedBooks[i];
            
            try {
                // Check if student already has issuedBooks in the response
                const existingIssuedBooks = this.getStudentIssuedBooks(student);
                if (existingIssuedBooks.length > 0) {
                    console.log(`Student ${student.name} already has ${existingIssuedBooks.length} issued books in response`);
                    continue;
                }
                
                // Fetch issued books from transactions/issues API
                const issuedBooks = await this.fetchStudentIssuedBooks(student._id);
                student.issuedBooks = issuedBooks;
                
                // Update loading progress for large datasets
                if (studentsWithIssuedBooks.length > 5) {
                    const progress = Math.round(((i + 1) / studentsWithIssuedBooks.length) * 100);
                    tbody.innerHTML = `
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center justify-center">
                                    <div class="w-16 h-16 border-4 border-purple-500 border-t-transparent rounded-full animate-spin mb-4"></div>
                                    <p class="text-slate-600">Loading student data... (${progress}%)</p>
                                    <p class="text-sm text-slate-500">Fetching issued books information</p>
                                </div>
                            </td>
                        </tr>
                    `;
                ...
                console.error(`Error fetching issued books for student ${student._id}:`, error);
                student.issuedBooks = [];
            }
        }
        
        return studentsWithIssuedBooks;
    }

    // Calculate and update statistics - NEW METHOD
    calculateAndUpdateStats(students) {
        ...
        students.forEach(student => {
            ...
            if (issuedBooks.length > 0) {
                ...
                issuedBooks.forEach(issue => {
                    if (!issue) return;
                    const dueDate = issue.dueDate || issue.returnDueDate;
        ...
        document.getElementById('activeStudentsCount').textContent = activeStudentsCount.toLocaleString();
        document.getElementById('studentOverdueCount').textContent = overdueCount.toLocaleString();
    }

    // Render students table - UPDATED VERSION
    renderStudentsTable(students) {
                ...
                <tr>
                    <td colspan="7" class="px-6 py-12 text-center">
                        <div class="flex flex-col items-center justify-center">
                            ...
                            ${!this.searchQuery ? `
                                <button id="addFirstStudentBtn" class="mt-4 px-4 py-2 bg-gradient-to-r from-purple-600 to-blue-600 text-white rounded-lg hover:opacity-90 transition duration-200">
        ...
        students.forEach(student => {
            ...
            const hasOverdue = issuedBooks.some(issue => {
            ...
            html += `
                <tr class="hover:bg-slate-50 transition duration-150" data-student-id="${student._id}">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        <div class="text-xs text-slate-500">${student.alternateMobile || ''}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                            ${student.className || student.class || 'General'}
                        </span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center">
                            <div class="relative">
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${issuedCount > 0 ? 'bg-green-100 text-green-800' : 'bg-slate-100 text-slate-800'}">
                                    ${issuedCount} book${issuedCount !== 1 ? 's' : ''}
                                </span>
                                ${issuedCount > 0 ? `
                                    <div class="absolute -top-1 -right-1 w-4 h-4 bg-green-500 rounded-full flex items-center justify-center">
                                        <span class="text-white text-xs font-bold">${issuedCount}</span>
                                    </div>
                                ` : ''}
                            </div>
                            ${hasOverdue ? `
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center space-x-2">
                            <button class="view-student-btn bg-blue-100 border border-blue-300 px-3 py-2 rounded-xl text-blue-600 hover:text-blue-900 hover:bg-blue-200 transition duration-200" data-id="${student._id}" title="View Details">
                                <i class="fas fa-eye text-sm"></i>
                            </button>
                            <button class="edit-student-btn bg-emerald-100 border border-emerald-300 px-3 py-2 rounded-xl text-emerald-600 hover:text-emerald-900 hover:bg-emerald-200 transition duration-200" data-id="${student._id}" title="Edit">
                                <i class='bx bx-edit text-sm'></i>
                            </button>
                            <button class="delete-student-btn bg-rose-100 border border-rose-300 px-3 py-2 rounded-xl text-rose-600 hover:text-rose-700 hover:bg-rose-200 transition duration-200" data-id="${student._id}" title="Delete">
                                <i class='bx bx-trash text-sm'></i>
                            </button>
                            <button class="issue-book-btn bg-purple-100 border border-purple-300 px-3 py-2 rounded-xl text-purple-600 hover:text-purple-900 hover:bg-purple-200 transition duration-200" data-id="${student._id}" title="Issue Book">
                                <i class='bx bx-book-add text-sm'></i>
                            </button>
                        </div>
                    </td>
                </tr>
    ...
    // View student details - UPDATED to fetch issued books
    async viewStudentDetails(studentId) {
        try {
            // Fetch student details
            const studentResponse = await api.fetch(`${this.baseURL}/${studentId}`);
            const studentData = await studentResponse.json();
            
            if (!studentResponse.ok || !studentData.success) {
                throw new Error(studentData.message || 'Failed to load student details');
            }

            // Fetch issued books
            const issuedBooks = await this.fetchStudentIssuedBooks(studentId);
    ...
    // Show student details modal - UPDATED VERSION
            ...
            <div class="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden animate-fade-in">
                <div class="p-6 overflow-y-auto max-h-[85vh]">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <p class="text-slate-900">${student.createdAt ? new Date(student.createdAt).toLocaleDateString('en-US', {
                                    year: 'numeric',
                                    month: 'long',
                                    day: 'numeric'
                                }) : 'N/A'}</p>
                            ...
                            <div>
                                ...
                                <p class="text-slate-900">${new Date(student.updatedAt).toLocaleDateString('en-US', {
                                    year: 'numeric',
                                    month: 'long',
                                    day: 'numeric'
                                })}</p>
                    ...
                    <div class="mb-6">
                        ...
                        ${issuedCount > 0 ? this.renderStudentIssuedBooks(issuedBooks) : `
    ...
    // Render student's issued books - UPDATED VERSION
    renderStudentIssuedBooks(issues) {
        ...
        issues.forEach(issue => {
            ...
            const bookTitle = issue.book?.title || issue.title || 'Unknown Book';
            const bookAuthor = issue.book?.author || issue.author || 'Unknown Author';
            ...
            html += `
                <div class="flex items-center justify-between p-3 ${isOverdue ? 'bg-red-50 border border-red-200' : 'bg-slate-50'} rounded-lg">
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            <p class="font-medium text-slate-800">${bookTitle}</p>
                            <p class="text-sm text-slate-500">${bookAuthor}</p>
                            <p class="text-xs text-slate-400">Issued: ${issue.issueDate ? new Date(issue.issueDate).toLocaleDateString() : 'N/A'}</p>
                        </div>
                    </div>
                    <div class="text-right">
                        ...
                        <p class="text-xs text-slate-500">${dueDate ? `Due: ${new Date(dueDate).toLocaleDateString()}` : 'No due date'}</p>
                    </div>
                </div>
            `;
        });
    ...
    // Export students to Excel - UPDATED VERSION
